...
AlexeyESector::AlexeyESector(vector<AlexeyELayer*>& layers, Geom::Phi<float> phiMin):
  theLayers(layers),
  thePhiMin(phiMin) 
{}


AlexeyESector::~AlexeyESector(){
  for (vector<AlexeyELayer *>::const_iterator ilay = theLayers.begin();
...
AlexeyVolume6Faces * AlexeyESector::findVolume(const GlobalPoint & gp, double tolerance) const {
  ...
  for(vector<AlexeyELayer*>::const_reverse_iterator ilay = theLayers.rbegin();
