...
MatBRod::MatBRod(vector<MatBSlab*>& slabs, Geom::Phi<float> phiMin) :
  theSlabs(slabs),
  thePhiMin(phiMin),
  theBinFinder(0)
{
  // TOFIX
//   if (verbose.debugOut) cout << "Building MatBRod with " << theSlabs.size()
// 		  << " slabs, minPhi " << thePhiMin << endl;
  
  if (theSlabs.size()>1) { // Set the binfinder
    vector<double> zBorders;
    for (vector<MatBSlab *>::const_iterator islab = theSlabs.begin();
  ...
  // TOFIX
    ...
    theBinFinder = new MatBinFinders::GeneralBinFinderInZ<double>(zBorders);
  }
}
...
MatVolume6Faces * MatBRod::findVolume(const GlobalPoint & gp, double tolerance) const {
