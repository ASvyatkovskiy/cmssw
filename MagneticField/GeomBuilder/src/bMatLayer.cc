...
MatGeoBuilderFromDDD::bMatLayer::bMatLayer(handles::const_iterator begin,
  ...
  mlayer(0) 
{
  ...
  if (MatGeoBuilderFromDDD::debug) {
    ...
    matVolumeHandle::printUniqueNames(theVolumes.begin(), theVolumes.end());
  }
  ...
  if (size==1) { // Only one volume; this is the case for barrel
    ...
    if (MatGeoBuilderFromDDD::debug) std::cout <<"      Sector is just one volume." << std::endl;
    ...
    do {
      if (MatGeoBuilderFromDDD::debug) std::cout << (*secBegin)->name 
  ...
  if (size!=1) { // Build the 12 sectors
    ...
    for (int i = 0; i<12; ++i) {
      ...
      sectors[isec>=0?isec:isec+12] = bMatSector(theVolumes.begin()+((i)*offset),
					      theVolumes.begin()+((i+1)*offset));
    }
  }

  if (MatGeoBuilderFromDDD::debug) std::cout << "-----------------------" << std::endl;

}

MatGeoBuilderFromDDD::bMatLayer::~bMatLayer(){}

int MatGeoBuilderFromDDD::bMatLayer::bin(int i) const {
  i = i%size;
  return (i>=0?i:i+size);
}

// const MatGeoBuilderFromDDD::bSector &
// MatGeoBuilderFromDDD::bMatLayer::sector(int i) const {
//   i = i%12;
//   return sectors[i>=0?i:i+12];
// }


double MatGeoBuilderFromDDD::bMatLayer::minR() const {
  // ASSUMPTION: a layer is only 1 volume thick (by construction). 
  return theVolumes.front()->minR();
}

// double MatGeoBuilderFromDDD::bMatLayer::maxR() const {
//   // ASSUMPTION: a layer is only 1 volume thick (by construction). 
//   return theVolumes.front()->maxR();
// }

MatBLayer * MatGeoBuilderFromDDD::bMatLayer::buildMatBLayer() const {
  if (mlayer==0) {

    // If we have only one volume, do not build any MatBSector.
    if (sectors.size()==0) {
      if (MatGeoBuilderFromDDD::debug && size!=0) {
	std::cout << "ERROR: bMatLayer::buildMatBLayer, 0 sectors but "
	     << size << " volumes" << std::endl;
      }
      // Technically we might have only one bSector built and we would
      // not need a separate MatBLayer constructor...
      mlayer = new MatBLayer(theVolumes.front()->matVolume, minR());
    }

    // If we have several sectors, create the MatBSector
    std::vector<MatBSector*> mSectors;
    for (unsigned int i=0; i<sectors.size(); ++i) {
      mSectors.push_back(sectors[i].buildMatBSector());
    }
    mlayer = new MatBLayer(mSectors, minR());
  }
  return mlayer;
}
