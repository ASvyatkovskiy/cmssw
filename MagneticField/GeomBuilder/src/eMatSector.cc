...
MatGeoBuilderFromDDD::eMatSector::eMatSector(handles::const_iterator begin,
  ...
  msector(0)
{
  ...
  if (MatGeoBuilderFromDDD::debug) cout << "     Z layers: " << zmin << " " << zmax << endl;
  ...
  if (MatGeoBuilderFromDDD::debug) cout << "     Found " << zClust.size() << " clusters in Z, "
		  << " layers: " << endl;
  ...
  for (unsigned int i=0; i<zClust.size() - 1; ++i) {
    ...
    if (MatGeoBuilderFromDDD::debug) {
      ...
      matVolumeHandle::printUniqueNames(layStart, separ);
    }
    
    layers.push_back(eMatLayer(layStart, separ));
    layStart = separ;
  }
  {
    if (MatGeoBuilderFromDDD::debug) {
    ...
    layers.push_back(eMatLayer(separ, last));
  }
...
MatGeoBuilderFromDDD::eMatSector::~eMatSector(){}


MatESector* MatGeoBuilderFromDDD::eMatSector::buildMatESector() const{
  if (msector==0) {
    vector<MatELayer*> mLayers;
    for (vector<eMatLayer>::const_iterator lay = layers.begin();
	 lay!=layers.end(); ++lay) {
      mLayers.push_back((*lay).buildMatELayer());
    }
    msector = new MatESector(mLayers, theVolumes.front()->minPhi()); //FIXME
  }
  return msector;
}
