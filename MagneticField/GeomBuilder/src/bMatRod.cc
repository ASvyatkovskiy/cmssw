...
MatGeoBuilderFromDDD::bMatRod::~bMatRod(){}

//The ctor is in charge of finding slabs inside the rod.
MatGeoBuilderFromDDD::bMatRod::bMatRod(handles::const_iterator begin,
  ...
  mrod(0)
{
  ...
  if (MatGeoBuilderFromDDD::debug) std::cout << "     Z slabs: " << zmin << " " << zmax << std::endl;
  ...
  if (MatGeoBuilderFromDDD::debug) std::cout << "     Found " << zClust.size() << " clusters in Z, "
		  << " slabs: " << std::endl;
  ...
  for (unsigned int i=0; i<zClust.size() - 1; ++i) {
    ...
    if (MatGeoBuilderFromDDD::debug) {
      ...
      matVolumeHandle::printUniqueNames(slabStart, separ);
    }
    
    slabs.push_back(bMatSlab(slabStart, separ));
    slabStart = separ;
  }
  {
    if (MatGeoBuilderFromDDD::debug) {
      std::cout << "     Slab at: " << zClust.back() <<" elements: " << last-separ
	   << " unique volumes: ";
      matVolumeHandle::printUniqueNames(separ,last);
    }
    slabs.push_back(bMatSlab(separ, last));
  }

  // Check that all slabs have the same dphi.
  std::vector<bMatSlab>::const_iterator i = slabs.begin();
  Geom::Phi<float> phimax = (*i).maxPhi();
  Geom::Phi<float> phimin = (*i).minPhi();
  for (++i; i!= slabs.end(); ++i) { 
    if(fabs(phimax - (*i).maxPhi()) > 0.001 ||
       fabs(phimin - (*i).minPhi()) > 0.001){
      if (MatGeoBuilderFromDDD::debug) std::cout << "*** WARNING: slabs in this rod have different dphi!" <<std::endl;
    }
  }
}
...
MatBRod* MatGeoBuilderFromDDD::bMatRod::buildMatBRod() const{
  if (mrod==0) {
    std::vector<MatBSlab*> mSlabs;
    for (std::vector<bMatSlab>::const_iterator slab = slabs.begin();
	 slab!=slabs.end(); ++slab) {
      mSlabs.push_back((*slab).buildMatBSlab());
    }
    mrod = new MatBRod(mSlabs,slabs.front().minPhi()); //FIXME
  }
  return mrod;
}
