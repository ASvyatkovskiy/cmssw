...
MatGeoBuilderFromDDD::matVolumeHandle::~matVolumeHandle(){
  delete refPlane;
}

MatGeoBuilderFromDDD::matVolumeHandle::matVolumeHandle(const DDExpandedView &fv, bool expand2Pi)
  : name(fv.logicalPart().name().name()),
    ...
    center_(GlobalPoint(fv.translation().x()/cm,
			...
			fv.translation().z()/cm)),
    expand(expand2Pi),
    materialType(fv.logicalPart().material())
{
  for (int i=0; i<6; ++i) {
    isAssigned[i] = false;
  }
  ...
  if (solid.shape() == ddbox) {
    ...
    buildMatPseudoTrap(fv);
  } else if (solid.shape() == ddtrunctubs) {   
    buildMatTruncTubs(fv);
  } else {
    cout << "matVolumeHandle ctor: Unexpected solid: " << (int) solid.shape() << endl;
  }
  ...
  { // Extract the name of associated field file.
	...
	cout << "*** WARNING: volume has > 1 SpecPar " << pname << endl;
      ...
      string find="[copyNo]";
      std::size_t j;
      for ( ; (j = matFile.find(find)) != string::npos ; ) {
	stringstream conv;
	conv << setfill('0') << setw(2) << copyno;
	string repl;
	conv >> repl;
	matFile.replace(j, find.length(), repl);
      }
  ...
  if (MatGeoBuilderFromDDD::debug) {  
    ...
    cout << "Summary: " << name << " " << copyno
	 ...
	 << " Material= " << fv.logicalPart().material().name()
         << " materialType= " << materialType.name().name()
...
const Surface::GlobalPoint & MatGeoBuilderFromDDD::matVolumeHandle::center() const {
  return center_;
}

void MatGeoBuilderFromDDD::matVolumeHandle::referencePlane(const DDExpandedView &fv){
...
void MatGeoBuilderFromDDD::matVolumeHandle::buildMatPhiZSurf(double startPhi,
						       double deltaPhi,
						       double zhalf,
						       double rCentr) {
...
bool MatGeoBuilderFromDDD::matVolumeHandle::sameSurface(const Surface & s1, Sides which_side, float tolerance)
{
  //Check for null comparison
  if (&s1==(surfaces[which_side]).get()){
    if (MatGeoBuilderFromDDD::debug) cout << "      sameSurface: OK (same ptr)" << endl;
    return true;
  }
  ...
  if (p1!=0) {
    ...
    if (p2==0) {
      if (MatGeoBuilderFromDDD::debug) cout << "      sameSurface: different types" << endl;
      return false;
    }
    
    if ( (fabs(p1->normalVector().dot(p2->normalVector())) > maxtilt)
	 && (fabs((p1->toLocal(p2->position())).z()) < tolerance) ) {
      if (MatGeoBuilderFromDDD::debug) cout << "      sameSurface: OK "
		      << fabs(p1->normalVector().dot(p2->normalVector()))
		      << " " << fabs((p1->toLocal(p2->position())).z()) << endl;
      return true;
    } else{
      if (MatGeoBuilderFromDDD::debug) cout << "      sameSurface: not the same: "
  ...
  if (MatGeoBuilderFromDDD::debug) cout << "      sameSurface: unknown surfaces..." << endl;
  return false;
}



bool MatGeoBuilderFromDDD::matVolumeHandle::setSurface(const Surface & s1, Sides which_side) 
{
 //Check for null assignment
  ...
  if (isAssigned[which_side]) {
    if (&s1!=(surfaces[which_side]).get()){
      cout << "*** WARNING matVolumeHandle::setSurface: trying to reassign a surface to a different surface instance" << endl;
      return false;
    }
  } else {
    surfaces[which_side] = &s1;
    isAssigned[which_side] = true;
    if (MatGeoBuilderFromDDD::debug) cout << "     Volume " << name << " # " << copyno << " Assigned: " << (int) which_side << endl;
    return true;
  }
...
const Surface & 
MatGeoBuilderFromDDD::matVolumeHandle::surface(Sides which_side) const {
...
const Surface & 
MatGeoBuilderFromDDD::matVolumeHandle::surface(int which_side) const {
...
MatGeoBuilderFromDDD::matVolumeHandle::sides() const{
...
void MatGeoBuilderFromDDD::matVolumeHandle::printUniqueNames(handles::const_iterator begin, handles::const_iterator end ) {
...
#include "MagneticField/GeomBuilder/src/buildMatBox.icc"
#include "MagneticField/GeomBuilder/src/buildMatTrap.icc"
#include "MagneticField/GeomBuilder/src/buildMatTubs.icc"
#include "MagneticField/GeomBuilder/src/buildMatCons.icc"
#include "MagneticField/GeomBuilder/src/buildMatPseudoTrap.icc"
#include "MagneticField/GeomBuilder/src/buildMatTruncTubs.icc"
