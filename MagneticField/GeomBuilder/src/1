...
MatGeoBuilderFromDDD::bMatSector::bMatSector(){}

MatGeoBuilderFromDDD::bMatSector::~bMatSector(){}

// The ctor is in charge of finding rods inside the sector.
MatGeoBuilderFromDDD::bMatSector::bMatSector(handles::const_iterator begin,
					handles::const_iterator end) :
  volumes(begin,end),
  msector(0)
{
  if (MatGeoBuilderFromDDD::debug) cout << "   Sector at Phi  " <<  volumes.front()->center().phi() << " " 
  ...
  if (volumes.size() == 1) {
    if (MatGeoBuilderFromDDD::debug) { 
      cout << "   Rod at: 0 elements: " << end-begin
	   << " unique volumes: ";
      matVolumeHandle::printUniqueNames(begin,end);
    }
    rods.push_back(bMatRod(begin,end));
  } else {
    ...
    if (MatGeoBuilderFromDDD::debug) cout << "     Found " << phiClust.size() << " clusters in Phi, "
		    << " rods: " << endl;
	...
	if (MatGeoBuilderFromDDD::debug) cout << "*** WARNING: Z lenght mismatch by " << DZ-DZ1
			<< " " << DZ << " " << DZ1 << endl;

      }
      if (fabs(DZ-DZ1) > 0.25 ) { // FIXME hardcoded tolerance
	if (MatGeoBuilderFromDDD::debug) cout << "       Incomplete, use also next cluster: " 
			<< DZ << " " << DZ1 << " " << DZ-DZ1 << endl;
	DZ1 = 0.;
	continue;
      } else if (DZ1>DZ+0.05) { // Wrong: went past max lenght // FIXME hardcoded tolerance
	cout << " *** ERROR: bMatSector finding messed up." << endl;
	matVolumeHandle::printUniqueNames(rodStart, separ);
	DZ1 = 0.;
      } else {
	if (MatGeoBuilderFromDDD::debug) {
	...
	rods.push_back(bMatRod(rodStart, separ));
	rodStart = separ;
	DZ1 = 0.;
      }
    }

    if (MatGeoBuilderFromDDD::debug) cout << "-----------------------" << endl;

  }
}


MatBSector* MatGeoBuilderFromDDD::bMatSector::buildMatBSector() const{
  if (msector==0) {
    vector<MatBRod*> mRods;
    for (vector<bMatRod>::const_iterator rod = rods.begin();
	 rod!=rods.end(); ++rod) {
      mRods.push_back((*rod).buildMatBRod());
    }
    msector = new MatBSector(mRods, volumes.front()->minPhi()); //FIXME
  }
  return msector;
}
