  ...
  doPhiCorr(cfg.getParameter<bool>("doPhiCorr"))
{
 ...
 //for StA
  ...
  for (int i = 0; i < n_types; ++i) {
	...
	relres_qinvpt[i][j][k] = fs->make<TH1F>(prefix + "relres_qinvpt", "", 700, -5.*invpt_scale[j][k], 4.*invpt_scale[j][k]);
        relres_qinvpt_phi[i][j][k] = fs->make<TH2F>(prefix + "relres_qinvpt_phi", "", 700, -5.*invpt_scale[j][k], 4.*invpt_scale[j][k],66,-phi_range[k], phi_range[k]);
        relres_pt[i][j][k] = fs->make<TH1F>(prefix + "relres_pt", "", 400, -5.*invpt_scale[j][k], 4.*invpt_scale[j][k]);
        relres_pt_phi[i][j][k] = fs->make<TH2F>(prefix + "relres_pt_phi", "", 700, -5.*invpt_scale[j][k], 4.*invpt_scale[j][k],66,-phi_range[k], phi_range[k]);
        if (is_StA_run) {
	...
	pull_phi     [i][j][k] = fs->make<TH1F>(prefix + "pull_phi",      "", 100, -pull_scale, pull_scale);
        ...
        //chi2dofs
        chi2dof      [i][j][k] = fs->make<TH1F>(prefix + "chi2dof",       "", 40, 0,20);
        chi2dof_eta  [i][j][k] = fs->make<TH2F>(prefix + "chi2dof_eta",   "", 40, 0,20,100, -eta_range[k], eta_range[k]);
        chi2dof_phi  [i][j][k] = fs->make<TH2F>(prefix + "chi2dof_phi",   "", 40, 0,20,66, -phi_range[k], phi_range[k]);
...
void SegsInFits::analyze(const Event& event, const EventSetup& eSetup) {
  ...
  for (int i = 0; i < int(tks[hits]->size()); ++i) {
	...
	const HitPattern& hpj = tkj.hitPattern();
        ...
        //printf("Eta here %f \n",gens->at(0).eta());
	//int loc = locBin(fabs(tki.eta())); //gens->at(0));
	...
	rpc_hits[loc]->Fill(hpi.numberOfValidMuonRPCHits(), hpj.numberOfValidMuonRPCHits());
        ...
        if (doPhiCorr) { 
            phiMultiplicativeCorr[0] = 1.;//(1-(*ts[0]).charge()*cos((*ts[0]).phi())*0.25);
            phiMultiplicativeCorr[1] = (1-(*ts[1]).charge()*cos((*ts[1]).phi())*0.25);
        }
        //printf("Before %f, after %f \n",(*ts[0]).pt(),phiMultiplicativeCorr[0]*((*ts[0]).pt())); 
	...
	for (int k = 0; k < 2; ++k) {
	  ...
	  double genqinvpt = gen.charge()/gen.pt();
          ...
          //printf("Stupidity check phi %f, eta %f, pt %f\n",tkphi,tketa,tkpt);
          //printf("Fill this number: %f to that histo: relres_qinvpt[%d][%d][%d]\n",(tkqinvpt - genqinvpt)/genqinvpt,k,loc,ptbin);
